...
def _dbg(msg: str, rank: object = "?") -> None:
    ...
    print(f"[iris dist rank={rank}] {msg}", file=sys.stderr, flush=True)
...
class GlooBackend:
    ...
    def __init__(self) -> None:
        ...
        _dbg(
            f"creating isolated gloo PG on {master_addr}:{iris_port} (rank={self._rank}/{self._world_size})",
            self._rank,
        )
        ...
        try:
            ...
            _dbg(f"TCPStore creation failed on port {iris_port}: {e}", self._rank)
            raise
        ...
        try:
            ...
            _dbg(f"ProcessGroupGloo creation failed: {e}", self._rank)
            raise

        _dbg("isolated gloo PG ready", self._rank)
    ...
    def all_gather(self, data: Union[torch.Tensor, Any]) -> list:
        if isinstance(data, torch.Tensor):
            ...
            try:
                ...
                _dbg(f"allgather failed: {e}", self._rank)
                raise
            if data.is_cuda:
        ...
        if isinstance(data, torch.Tensor):
            ...
            if data.is_cuda:
                ...
                try:
                    ...
                    _dbg(f"broadcast (tensor) failed: {e}", self._rank)
                    raise
                ...
                try:
                    ...
                    _dbg(f"broadcast (cpu tensor) failed: {e}", self._rank)
                    raise
            return data
    ...
    def host_barrier(self, stream=None) -> None:
        try:
            ...
            _dbg(f"barrier failed: {e}", self._rank)
            raise
    ...
    def send(self, tensor: torch.Tensor, dst: int) -> None:
        ...
        try:
            ...
            _dbg(f"send(dst={dst}) failed: {e}", self._rank)
            raise
    ...
    def recv(self, tensor: torch.Tensor, src: int) -> None:
        ...
        try:
            ...
            _dbg(f"recv(src={src}) failed: {e}", self._rank)
            raise
