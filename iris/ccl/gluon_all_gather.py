...
world_size stores per tile while staying within gluon's 1D BlockedLayout
framework.

Start and end barriers are fused into the kernel (same protocol as the gluon
all-reduce): the start barrier guarantees every peer's output buffer is ready
before remote stores begin; the end barrier guarantees the local output is
fully written by all peers when the kernel exits. Without the end barrier the
output is not yet valid at kernel completion and the skew leaks into the next
collective.
"""
...
from .gluon_all_reduce import _gluon_barrier
...
def persistent_all_gather_gluon(
    ...
    Each rank stores its input tile to every rank's output buffer at row
    offset group_rank * M, matching torch.distributed.all_gather_into_tensor
    semantics. Start and end barriers bound cross-rank skew inside the kernel.
    """
...
def launch(
    ...
    Graph-capture safe: in eager mode, zeros barrier flags. During graph
    capture, skips the zero — flags accumulate across replays and the
    ...
    if workspace is None or not hasattr(workspace, "start_flags"):
        workspace = _GluonAllGatherWorkspace(ctx, world_size)
...
def all_gather_preamble(output_tensor, input_tensor, ctx, config=None, workspace=None):
    """Allocate barrier flag tensors for the gluon all-gather."""
...
class _GluonAllGatherWorkspace:
    """Holds barrier flag tensors for the gluon all-gather."""
