...
class SymmetricHeap:
    ...
    def validate(self):
        ...
        for r, b in enumerate(bases):
            assert b != 0, f"[rank={rank}] heap_bases[{r}] is null. All bases: {[hex(x) for x in bases]}"

        # 3. All heap bases are unique (no two ranks mapped to the same VA)
        assert len(set(bases)) == self.num_ranks, f"[rank={rank}] Duplicate heap bases: {[hex(x) for x in bases]}"

        # 4. Local heap_bases entry matches allocator base
        assert bases[rank] == base, f"[rank={rank}] heap_bases[{rank}]={hex(bases[rank])} != allocator base={hex(base)}"
        ...
        if hasattr(self, "_peer_va_ranges"):
            ...
            for peer, peer_va in self._peer_va_ranges.items():
                assert not (base <= peer_va < local_end), (
                    f"[rank={rank}] Peer {peer} VA {hex(peer_va)} overlaps local heap [{hex(base)}, {hex(local_end)})"
                )
    ...
    def refresh_peer_access(self):
        ...
        for peer, sock in self.fd_conns.items():
            ...
            if peer not in self._peer_va_ranges:
                ...
                assert peer_va_base != 0, f"mem_address_reserve returned null VA for peer={peer} (rank={self.cur_rank})"
